...
- **Snapshot**: A Markdown file containing the extracted text content of a rendered view or Slack message, committed to the repository as a baseline.
- **Slack Message Snapshot**: A Markdown file generated by calling backend Slack formatting functions (deployment notifications and change announcements) with test data and converting the output from Block Kit / mrkdwn to standard Markdown.
